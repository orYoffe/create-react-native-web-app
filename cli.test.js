...
describe('cli runs properly', () => {
  it('cli runs with argument and logs info', () => {
    ...
    expect(chalk.cyan.mock.calls).toEqual([['⏳ Creating React Native Web App by the name of myFakeName ...'], ['✅ Created project folder.'], ['✅ Added project files.'], ['⏳ Installing project dependencies...'], ['✅ Installed project dependencies.'], ['cd myFakeName'], ['Then run the these commands to get started:'], ['npm run web'], ['npm run android'], ['npm run ios'], ['npm run test'], ['npm run build']]);
    ...
    expect(execSync.mock.calls).toEqual([
      ['cd myFakeName && npx react-native-rename myFakeName && npm i', { stdio: [0, 1, 2] }],
    ]);

    expect(fs.emptyDirSync.mock.calls).toEqual([['myFakeName']]);
    expect(fs.ensureDirSync.mock.calls).toEqual([['myFakeName']]);
    expect(fs.copySync.mock.calls).toEqual([[__dirname + (isWin ? '\\' :'/' ) + 'template', 'myFakeName']]);
